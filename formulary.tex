...
\subsection*{Poisson}
Carta \textit{c} e carta \textit{u} - usata per eventi rari
\begin{equation*}
...
\begin{center}
    \begin{tabular}{|c|c|}
        ...
        \(np \geq 5 \land  0.1<p<0.9\) & Binomiale \(\rightarrow\) Normale\\\hline
        \(\lambda > 15\) & Poisson \(\rightarrow\) Normale\\\hline
    \end{tabular}
\end{center}
...
M'' &= \phi\left[\left(\frac{LSI-LSS}{\sigma}+k\right)\sqrt{\frac{n}{n-1}}\right]\\
M = M' + M''\\
\end{align*}
\begin{equation*}
    \boxed{\widehat{p} < M}
...
\begin{center}
    \begin{tabularx}{\linewidth}{|c|c|X|c|}
        \hline
       TRASFORMAZIONI & TIPOLOGIA & \multicolumn{1}{c|}{ESEMPI} & test \\\hline
       ...
       \makecell{Lineare \\ \(\phi (x)=\alpha x + \beta\)} & Intervallo & temperatura, calendario & t, F\\\hline
       \(x \geq y \Leftrightarrow \phi(x)\geq\phi(y)\) & Ordinale & classifiche, qualitá dell'aria, durezza & segno\\\hline
       Qualsiasi a uno a uno & Nominale & maglie giocatori, colore occhi & \(\chi ^2\)\\\hline
    \end{tabularx}
\end{center}
...
\subsection*{Indice \(\mathbf{C_{pk}}\)}
...
\subsection*{Difettositá}

\begin{align*}
    p &= \phi(-3C_{pi})+\phi(-3C_{ps})\\
    p &= 2\phi(-6C_{p}+3C_{pk})
\end{align*}
